...
RUN printf "#!/bin/bash\naws ec2 describe-instances --query 'Reservations[*].{ID:Instances[*].InstanceId,TAG:Instances[*].Tags}' | jq .\n" > /usr/local/bin/awsls
...
RUN printf "#!/bin/bash\naws ec2 terminate-instances --instance-ids $1" > /usr/local/bin/awsterminate
